...
#include<optional>
#include <set>
#include <fstream> // to load binary data
...
#include <glm/glm.hpp> //linear algebra stuff
...
public:
    void run() {
        ...
        initVulkan();
        mainLoop();
        std::cout << "GOODBYE. oh and btw you smell." << std::endl;
        cleanup();
    }
...
private: // Where all functions and Vulkan objects will be stored
    ...
    VkPhysicalDevice physicalDevice = VK_NULL_HANDLE; // device stuff
    VkDevice device;
    VkPhysicalDeviceFeatures deviceFeatures{};

    VkSwapchainKHR swapChain; // swap chain stuff
    ...
    const int MAX_FRAMES_IN_FLIGHT = 2; // oog
    ...
    // command pools -- for queuing draw commands
    VkCommandPool commandPool; // hell yeah baby
    std::vector<VkCommandBuffer> commandBuffers; //now we're gaming
    ...
    void initWindow() {
        ...
        glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API); // The window will not use OpenGL
        glfwWindowHint(GLFW_RESIZABLE, GLFW_FALSE); // Disable resizing

        window = glfwCreateWindow(WIDTH, HEIGHT, "Vulkan", nullptr, nullptr); // Create window as callable object
    }
    ...
    void createInstance() {
        ...
        // The following code tells the computer some info about the program
        ...
        // The following code creates a vulkan instance using the appinfo from earlier
        VkInstanceCreateInfo createInfo{};
        createInfo.sType = VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO;
        createInfo.pApplicationInfo = &appInfo;

        // Set desired global extensions from required extensions
        auto extensions = getRequiredExtensions();
        createInfo.enabledExtensionCount = static_cast<uint32_t>(extensions.size());
        createInfo.ppEnabledExtensionNames = extensions.data();

        // Modify createInfo to include validation layer names if available
        ...
        // Setup Debug messenger for this function specifically
    ...
    void pickPhysicalDevice() {
        // lists available graphics cards
        uint32_t deviceCount = 0;
        vkEnumeratePhysicalDevices(instance, &deviceCount, nullptr);

        std::cout << deviceCount << " physical graphics devices detected." << std::endl;

        // if no devices can support vulkan then there is no point
        if (deviceCount == 0) {
            throw std::runtime_error("failed to find GPUs with Vulkan support!");
        }

        // Allocate array for all physical device handles
        std::vector<VkPhysicalDevice> devices(deviceCount); //error IS here!!! there are 2 physical devices
        vkEnumeratePhysicalDevices(instance, &deviceCount, devices.data()); //or here

        // Rank devices on suitablility
        ...
        //tell user what GPU they are using
        VkPhysicalDeviceProperties currentGPUProperties;
        vkGetPhysicalDeviceProperties(physicalDevice, &currentGPUProperties);
        std::cout << "Currently using " << currentGPUProperties.deviceName << std::endl;
    }
    ...
    int rateDeviceSuitability(VkPhysicalDevice device) {
        ...
        if (deviceProperties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU) {
            score += 1000;
            SetConsoleColor(95);
            std::cout << "DEDICATED GPU DETECTED. INJECTING GAMER GUNK INTO LARGE PULSING FOREHEAD VAIN." << std::endl;
            ResetConsoleColor();
        }
        ...
        // Application can't run on a GPU without desired features
        if (!isDeviceSuitable(device)) {
            return 0;
        }
        std::cout << deviceProperties.deviceName << " scored: " << score << std::endl;
    ...
    void createLogicalDevice() {
        ...
        // Set device specific Validation Layer names with compatibility in mind
    ...
    VkSurfaceFormatKHR chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
        /* Set the render surface format
        * The VideoCore IV supports 16-bit HDR, and we will push it to its limit.
        * "1G pixels/s with single bilinear texturing, simple shading, 4x multisampling." (Broadcom, 2013, VideoCore® IV 3D Architecture Reference Guide)
        */

        // TODO: CHANGE TO 16-bit HDR!!!!!!!!
        for (const auto& availableFormat : availableFormats) {
            if (availableFormat.format == VK_FORMAT_B8G8R8A8_SRGB && availableFormat.colorSpace == VK_COLOR_SPACE_SRGB_NONLINEAR_KHR) { // this is 8 bit, change to 16 later.
    ...
    VkPresentModeKHR chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) {
        // Vsync, this will be a family console on a low spec system so its fine.
        return VK_PRESENT_MODE_FIFO_KHR;
        // normally you would check for higher performance stuff here but we dont care about that.
    }

    VkExtent2D chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
        // this is basically the resolution of the swap chain, however, the VideoCore IV uses "tile-based pixel rendering", so keep that in mind.

        // this code checks to see if the virtual screen is the same size as the real screen. Our system is clamped at 720p, and the monitor upscales for us.
        // this code would be pointless but it helps with debugging.
    ...
    void createSwapChain() {
        ...
        // set swap chain image count to 1 + the minimum
        uint32_t imageCount = swapChainSupport.capabilities.minImageCount + 1;

        // unless that is greater than the max (very likely for us)
    ...
    void createGraphicsPipeline() {
        // This function creates a graphics pipeline:
        /*
        ...
        */
        ...
        // shaders need a pipeline to pass through in order to work.
        ...
        // Dynamic state stuff does not need to happen because the window size is always the same and nothing ever changes.
        ...
        // input assembly describes what kind of geometry will be drawn to the screen.
        VkPipelineInputAssemblyStateCreateInfo inputAssembly{};
        inputAssembly.sType = VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO;
        inputAssembly.topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST; // we will be using VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP to optimise later
        inputAssembly.primitiveRestartEnable = VK_FALSE;

        // Viewport shennanigans
        VkViewport viewport{};
        viewport.x = 0.0f;
        viewport.y = 0.0f;
        ...
        // the scissor is basically a mask for the image
        VkRect2D scissor{};
        scissor.offset = { 0, 0 };
        ...
        rasterizer.depthClampEnable = VK_FALSE; // we will want this to be true later

        rasterizer.rasterizerDiscardEnable = VK_FALSE;

        rasterizer.polygonMode = VK_POLYGON_MODE_FILL; //this would be cool to change as a cheat setting
        /*
        ...
        */
        rasterizer.lineWidth = 1.0f; //widelines needs to be enabled for anything wider than 1 pixel
        rasterizer.cullMode = VK_CULL_MODE_BACK_BIT;
        rasterizer.frontFace = VK_FRONT_FACE_COUNTER_CLOCKWISE;

        // for shadow stuff (idk if we will have shadows)
        rasterizer.depthBiasEnable = VK_FALSE;
        rasterizer.depthBiasConstantFactor = 0.0f; // Optional
        rasterizer.depthBiasClamp = 0.0f; // Optional
        rasterizer.depthBiasSlopeFactor = 0.0f; // Optional

        //multisampling, videocore IV can do 4x AND simple shading at 720p
        ...
        // we can do colour blending no problem, if it becomes a problem then we ignore it and die.
        ...
        //VK gaohienfjkn ahwua h(may be in wrong spot)
    ...
    static std::vector<char> readFile(const std::string& filename) {
        ...
        std::cout << "\033[33mReading file [" << filename << "] of size " << fileSize << "bytes \033[0m" << std::endl;
        if (!file.is_open()) {
    ...
    VkShaderModule createShaderModule(const std::vector<char>& code) {
        // some code to take the loaded compiled shaders and make them a format vulkan can understand.
        VkShaderModuleCreateInfo createInfo{};
        createInfo.sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO;
        createInfo.codeSize = code.size();
        createInfo.pCode = reinterpret_cast<const uint32_t*>(code.data());

        // create shader module to return
    ...
    void createRenderPass() {
        // attachment description is single colour buffer
        ...
        // store operation puts rendered triangle on screen
    ...
    void createFramebuffers() {
        swapChainFramebuffers.resize(swapChainImageViews.size()); // make frame buffers the amount set earlier

        //iterate through image views and make frame buffers from them
    ...
    // recreate swap chain if something goes wrong
    void recreateSwapChain() {
        ...
        vkDeviceWaitIdle(device); // never touch resources that are still in use

        cleanupSwapChain(); // clean up first
    ...
    void drawFrame() {
        /*
        * Basically, it do this:
        * 1. wait for last image 2 finsih
        * 2. yoink image from swap chain
        * 3. record command buffer (a list of draw to image commands)
        ...
        */
        ...
        //2
        uint32_t imageIndex; //ugg this may be in the wrong spot
        VkResult result = vkAcquireNextImageKHR(device, swapChain, UINT64_MAX, imageAvailableSemaphores[currentFrame], VK_NULL_HANDLE, &imageIndex);

        // check to make sure that the surface has not changed
    ...
    void createTextureSampler() {
        ...
        samplerInfo.anisotropyEnable = VK_TRUE; // TODO: set to false for RPI
    ...
    void initVulkan() {
        ...
        createGraphicsPipeline(); // big one
        createDepthResources();
    ...
    // Console Colour Functions I shamelessly stole
    ...
    void mainLoop() {
        while (!glfwWindowShouldClose(window)) { // Check for window close input
            glfwPollEvents(); // while the window shouldnt be closing, do GLFW stuff
            drawFrame(); // what do you think this does?
        }
